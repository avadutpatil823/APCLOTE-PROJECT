...
const getGridTone = ({ submitted, isCurrent, isAnswered, isCorrect, isWrong }) => {
  ...
  if (isCurrent) {
    return {
      bg: alpha("#38bdf8", 0.22),
      border: alpha("#67e8f9", 0.42),
      color: "#e0f2fe",
    };
  }
...
const extractJsonPayload = (value) => {
  ...
  try {
    ...
    console.debug("Quiz JSON parse failed:", error);
    return null;
  }
};
...
const AIMentorQuiz = () => {
  ...
  if (!quizStarted) {
    ...
    return (
      <Box className="flex h-full items-center justify-center">
        <Paper
          elevation={0}
          className="w-full max-w-4xl rounded-[30px] border border-white/10 px-5 py-6 md:px-7 md:py-8"
          sx={{
            bgcolor: alpha("#0f172a", 0.8),
            boxShadow: `0 28px 80px ${alpha("#020617", 0.26)}`,
          }}
        >
          <Box className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
            <Box>
              ...
              <Typography sx={{ mt: 2.2, color: "#f8fafc", fontSize: 32, fontWeight: 800 }}>
                Start a timed quiz with Dummi X
              </Typography>
              <Typography sx={{ mt: 1.5, color: "rgba(226,232,240,0.74)", lineHeight: 1.85 }}>
            ...
            <Paper
              ...
              sx={{
                ...
                border: `1px solid ${alpha("#67e8f9", 0.1)}`,
              }}
            >
              <Typography sx={{ color: "#f8fafc", fontSize: 18, fontWeight: 800 }}>
                Quiz Rules
              </Typography>
  ...
  return (
    <Box ref={quizContainerRef} className="h-full">
      <Box className="mx-auto flex h-full w-full max-w-6xl flex-col gap-4 lg:flex-row">
        <Paper
          elevation={0}
          ...
          sx={{
            p: 2.2,
            borderRadius: 4,
            bgcolor: alpha("#0f172a", 0.84),
            border: `1px solid ${alpha("#67e8f9", 0.12)}`,
            boxShadow: `0 18px 42px ${alpha("#020617", 0.2)}`,
          }}
        >
          <Typography sx={{ color: "#f8fafc", fontSize: 18, fontWeight: 800 }}>
            Quiz Map
          </Typography>
          <Typography sx={{ mt: 0.8, color: "rgba(226,232,240,0.7)", lineHeight: 1.7 }}>
        ...
        <Paper
          ...
          sx={{
            ...
            border: `1px solid ${alpha("#67e8f9", 0.12)}`,
            boxShadow: `0 24px 56px ${alpha("#020617", 0.24)}`,
          }}
        >
          <Box className="flex flex-col gap-4 border-b border-white/8 pb-5 lg:flex-row lg:items-start lg:justify-between">
            <Box>
              <Typography sx={{ color: "#f8fafc", fontSize: 28, fontWeight: 800 }}>
                {quizTitle}
              </Typography>
              <Typography sx={{ mt: 1, color: "rgba(226,232,240,0.72)", lineHeight: 1.75 }}>
          ...
          {quizError ? (
            <Paper
              ...
              sx={{
                ...
                color: didAutoSubmit ? "#fde68a" : "#bae6fd",
          ...
          {submitted ? (
            <Box className="mt-5 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
              {[
                { label: "Attempted", value: result.attempted, color: "#bae6fd", bg: "#0ea5e9" },
                { label: "Correct", value: result.correct, color: "#bbf7d0", bg: "#22c55e" },
                { label: "Wrong", value: result.wrong, color: "#fecaca", bg: "#ef4444" },
                { label: "Skipped", value: result.skipped, color: "#fde68a", bg: "#f59e0b" },
              ].map((item) => (
                <Paper
                  key={item.label}
                  elevation={0}
            ...
            >
              ...
              <Stack spacing={1.8} sx={{ mt: 3 }}>
                {QUIZ_OPTION_KEYS.map((optionKey, index) => {
                  ...
                  let optionBg = alpha("#ffffff", 0.03);
                  let optionBorder = alpha("#ffffff", 0.08);
                  let optionTextColor = "#e2e8f0";

                  if (isSelected && !submitted) {
                    optionBg = alpha("#38bdf8", 0.16);
                    optionBorder = alpha("#38bdf8", 0.32);
                    optionTextColor = "#e0f2fe";
                  }

                  if (isCorrect) {
                    optionBg = alpha("#22c55e", 0.16);
                    optionBorder = alpha("#22c55e", 0.34);
                    optionTextColor = "#dcfce7";
                  }

                  if (isWrongSelection) {
                    optionBg = alpha("#ef4444", 0.16);
                    optionBorder = alpha("#ef4444", 0.34);
                    optionTextColor = "#fee2e2";
                  }
                  ...
                  return (
                    ...
                    >
                      ...
                      {submitted && isCorrect ? (
                        <Typography sx={{ color: "#bbf7d0", fontWeight: 800 }}>Correct</Typography>
                      ) : null}
                      {submitted && isWrongSelection ? (
                        <Typography sx={{ color: "#fecaca", fontWeight: 800 }}>Your pick</Typography>
                      ) : null}
                    </Button>
                  );
                })}
              ...
              <Box className="mt-6 flex flex-col gap-3 border-t border-white/8 pt-5 sm:flex-row sm:items-center sm:justify-between">
