...
export const lecturerUserReducer=(state=initialState,action)=>{
    switch(action.type){
        ...
        case GETALL_LEECTURERS_SUCCESS:
            return {...state,isloading:false,error:null,lecturers:action.payload.content,page:action.payload.number+1,totalPages:action.payload.totalPages}
        case GETALL_LEECTURERS_FAILURE:
            return {...state,isloading:false,error:action.payload}
        case GETALL_USERS_REQUEST:
            return {...state,isloading:true,error:null}
        case GETALL_USERS_SUCCESS:
            return {...state,isloading:false,error:null,users:action.payload.content,page:action.payload.number+1,totalPages:action.payload.totalPages}
        ...
        case GETALL_STUDENTS_SUCCESS:
            return {...state,isloading:false,error:null,students:action.payload.content,page:action.payload.number+1,totalPages:action.payload.totalPages}
