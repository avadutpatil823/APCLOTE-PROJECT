...
export const createClassrOOM=(name,batchId)=>async (dispatch)=>{
    ...
    try{
   const response= await axios.get(buildUrl(`/lecturer/createClassRoom?name=${encodeURIComponent(name)}&batchId=${batchId}`),{headers:{
...
export const createClass=(class1)=>async (dispatch)=>{
   ...
   const response= await axios.post(buildUrl("/lecturer/createClass"),class1,{headers:{
    ...
    return true
        ...
        dispatch({type:CREATECLASS_FAILURE,payload:error?.response?.data?.message || "Failed To Create Class"})
...
export const uploadVideo = (formData) => async (dispatch) => {
  ...
  try {
    ...
    dispatch({
      ...
      payload: "Video Uploaded Successfully"
    });
