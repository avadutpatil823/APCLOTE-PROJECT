...
export const getAllBatchs=(pageNumber)=>async (dispatch)=>{
    ...
    try{
   const response= await axios.get(`http://localhost:9898/api/getAllBatchs?pageNumber=${pageNumber}&pageSize=20`)
...
export const getMyBatchs=()=>async (dispatch)=>{
    ...
    try{
   const response= await axios.get("http://localhost:9898/api/getMyBatchs",{headers:{
...
export const getLecturerBatchs=()=>async (dispatch)=>{
    ...
    try{
   const response= await axios.get("http://localhost:9898/lecturer/getMyBatchs",{headers:{
      "Content-Type": "application/json",
      "Authorization": `Bearer ${JSON.parse(localStorage.getItem("JWT"))}`
    }})
...
export const getSearchedBatchs=(key)=>async (dispatch)=>{
    ...
    try{
   const response= await axios.get(`http://localhost:9898/api/search?key=${key}`)
    console.log(response.data)
...
export const getSubjects=()=>async (dispatch)=>{
    ...
    try{
   const response= await axios.get("http://localhost:9898/admin/getAllSubjects",{headers:{
      "Content-Type": "application/json",
      "Authorization": `Bearer ${JSON.parse(localStorage.getItem("JWT"))}`
    }})
    console.log(response.data)
...
export const getCourses=()=>async (dispatch)=>{
    ...
    try{
   const response= await axios.get("http://localhost:9898/admin/getAllCourses",{headers:{
      "Content-Type": "application/json",
      "Authorization": `Bearer ${JSON.parse(localStorage.getItem("JWT"))}`
    }})
    console.log(response.data)
